...
const CadMassa = () => {
  ...
  const enviar = async () => {
    ...
    try {
        await axios.post(`${api}/api/v1/cadXML/uploadfile/`,
            formData, {
            headers: {
  ...
  const preview = async () => {
    ...
    try {
        const response = await axios.post(`${api}/api/v1/cadXML/previewfile/`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
	...
	return (
		<div className={styles.container}>
          ...
          <div className={styles.footer}>
           <button onClick={enviar} className={styles.send}>Cadastrar</button>
           <button onClick={preview} className={styles.send}>Visualização</button>
          </div>
				</div>
			</div>
		</div>
